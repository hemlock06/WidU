...
const decisions = [
  ...
  ["D40", "기기 구성 벤치마크(워치만/폰만/둘다) — 융합 정책 검증", "UMAFall 동기 손목·허리(피험자 분리 5-fold, 누수 차단)로 측정: 워치만 F1 0.899(오탐 9.1%)·폰만 F1 0.987(오탐 0.5%)·둘다 OR recall 0.995(189중 1개만 놓침). ★양쪽 동시발화=ADL 오탐 0/428(교차확인 고신뢰). 폰(허리)이 본질 정밀, 워치 주력은 가용성. → 계층형 융합(양쪽발화 즉시격상·워치단독 self-check·폰단독 경량확인) = 기존 L5 설계 정량 검증. 고령 recall 갭은 불변(데이터 문제)."],
];
