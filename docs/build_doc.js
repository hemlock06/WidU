...
children.push(Bul("**★iOS 네이티브 낙상 알림 미가용**: iOS는 애플워치가 검출한 낙상 '알림'을 앱이 가져올 수 없다(센서 원본만 접근). 따라서 iOS에선 네이티브 위임 경로가 사실상 막힘 → **우리 L2 모델이 iOS 주력**, 네이티브 위임은 이벤트를 노출하는 플랫폼(삼성 Wear OS 등)에 한해 보강. 기존에 '네이티브 위임이 주력'이라 가정한 부분(pipeline.ingest_fall_event)을 정정 — iOS에선 손목 LODO recall 갭(0.74)이 더 부담이라 L5 교차검증·미회복 net이 그만큼 중요."));
...
children.push(Bul("**★iOS 네이티브 낙상 미가용(v0.21)**: iOS는 애플워치 낙상 '알림'을 앱이 못 가져옴(센서 원본만) → iOS에선 네이티브 위임 불가, 우리 L2 모델이 주력. 손목 recall 갭(0.74)이 iOS에서 더 부담 → L5 교차검증·미회복 net·능동학습으로 보완. (삼성 Wear OS 등은 네이티브 보강 가능.)"));
...
children.push(table(["일자", "버전", "변경/결정", "비고"], [
  ...
  [today, "v0.22", "기기 구성 벤치마크(워치만/폰만/둘다). UMAFall 동기 손목·허리·피험자분리. 워치만 F1 0.899(오탐9.1%)·폰만 0.987(0.5%)·둘다 OR recall 0.995. ★양쪽발화 ADL오탐 0/428. → 계층형 융합이 목적 부합(기존 L5 검증)", "D40. benchmark_device_configs.py(신규, 분석전용). 폰=본질정밀·워치=가용성 주력. 고령 recall 갭은 불변(데이터 문제). 서빙 코드 무변경(golden 11/11 유지)."],
], [1300, 900, 5160, 2000]));
children.push(P(""));
