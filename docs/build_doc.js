...
children.push(table(["항목", "내용"], [
  ["문서명", "WidU 해설문서"],
  ["버전", "v0.24 (앱 코드 직접 확인 — IMU 미구축 실태 + 전송 스펙·레이트 강건성)"],
  ["작성일", today],
...
const decisions = [
  ...
  ["D39", "WIDYU 프론트 통화(6/30) 반영 — iOS 네이티브 낙상 미가용·전송 계약", "스테이크홀더 통화로 진짜 델타만 반영(대부분 이미 일치: 위치별·신뢰도게이트·둘다운용). ★iOS는 애플워치 낙상 '알림'을 앱이 못 가져옴(센서 원본만) → iOS는 우리 L2 모델 *주력*(네이티브 위임은 삼성 Wear OS 등 한정). ★전송 계약 워치 1초/폰 3초 배치 — 낙상은 충격 전후 고주파창이 배치에 담겨야 동작. 개인화 3일=L1 EWMA가 의도 포함. 코드 로직 불변(주석·README·brain 정정)."],
  ["D42", "앱 코드 직접 확인 — 디바이스 IMU 미구축(현 최대 갭) + 전송 스펙·레이트 강건성", "hemlo '우리 깃 코드 보면 되잖아' → GB-able/WIDYU-widyu(Flutter) 직접 확인. ★현재 앱은 IMU(가속도/자이로)를 어디서도 수집/전송 안 함, 심박만(Wear OS Health Services). 애플워치 앱 미완(WIP), 현 워치=Wear OS. → L2 낙상은 라이브 데이터 소스 부재(iOS API 논쟁은 시기상조, 1순위=IMU 파이프라인). IMU는 계획만(워치 1초/폰 3초 배치·원본). ★전송 핵심=간격 아닌 배치 내 ≥50Hz. 레이트 강건성 측정: 50Hz=벤치마크와 동일(성능 유지)·워치는 25Hz서 recall 0.984→0.91·12.5Hz서 0.72 붕괴(폰·둘다는 강건). → 워치 반드시 ≥50Hz. 헴록 교훈: 웹문서+구두 2회 번복을 소스코드가 단번에 확정."],
  ["D41", "iOS 네이티브 낙상 — '미가용'→'watchOS 앱 조건부 가능' 정정(애플 1차 문서)", "D39의 '미가용'은 김민지 구두 확인만 출처(파생). 애플 1차 문서 직접 검증: iPhone 앱 단독으론 불가하나 ★watchOS 컴패니언 앱이면 가능 — ①CMFallDetectionManager 낙상 이벤트 수신(엔타이틀먼트 com.apple.developer.health.fall-detection 애플 승인 필요·재량) ②CMBatchedSensorManager 원본 IMU 고주파 배치(800Hz, 전송계약과 합치). 진짜 병목=watchOS 앱 유무. 미확인=승인·24/7 지속성·앱 유무(김민지 확인 대기). 승인 안 나도 우리 L2 동작 → 모델이 주력/안전망. 헴록 교훈: 파생출처 '확정'을 1차 소스가 정정."],
  ["D40", "기기 구성 벤치마크(워치만/폰만/둘다) — 융합 정책 검증", "UMAFall 동기 손목·허리(피험자 분리 5-fold, 누수 차단)로 측정: 워치만 F1 0.899(오탐 9.1%)·폰만 F1 0.987(오탐 0.5%)·둘다 OR recall 0.995(189중 1개만 놓침). ★양쪽 동시발화=ADL 오탐 0/428(교차확인 고신뢰). 폰(허리)이 본질 정밀, 워치 주력은 가용성. → 계층형 융합 = 기존 L5 설계 정량 검증. 고령 recall 갭은 불변(데이터 문제). ★후속: '회색지대 교차확인 격상' 가설 측정 → 양쪽 회색 동시 2 trial·전부 ADL·낙상 0 → 기각, self_check 현 설계 유지(과적합 회피)."],
];
...
children.push(H2("6-19. WIDYU 프론트 통화 반영 — 디바이스/플랫폼 제약 (v0.21)"));
children.push(P("2026-06-30 프론트엔드 개발자 통화. WidU 설계가 합의된 AI 로직과 대부분 이미 일치함을 확인하고, 어긋난 2건만 정정. (코드 동작은 불변 — 주석·문서 정렬.)", { italics: true }));
children.push(P("① 이미 일치(반영 불필요):", { bold: true }));
children.push(Bul("**위치별 분리**(폰=허리/워치=손목) = D21~D22. **신뢰도 게이트**(심박 이상 시 자이로 충격 2차 확인) = L2 캐스케이드(IMPACT_HARD_G+무활동). **둘 다 운용·기기 통일** = 기기 가용성 매트릭스/LODO. **개인화 ~3일 축적**(인원별 평균 기준점) = L1 EWMA가 이 의도를 이미 포함."));
children.push(P("② 정정 2건(반영함):", { bold: true }));
children.push(Bul("**★iOS 네이티브 낙상 — 통화 시점 '미가용'으로 판단**: iOS는 애플워치가 검출한 낙상 '알림'을 앱이 가져올 수 없다(센서 원본만 접근)는 게 통화 결론이었음. → 당시엔 '우리 L2 모델이 iOS 주력'으로 정정. ※이 항목은 v0.23에서 애플 1차 문서 검증으로 재정정됨(아래·D41)."));
children.push(Bul("**(v0.23 재정정) 실은 'iPhone 앱 단독 불가 / watchOS 앱이면 조건부 가능'**: 애플 문서 직접 확인 — ①`CMFallDetectionManager`로 낙상 이벤트 수신(엔타이틀먼트 com.apple.developer.health.fall-detection 애플 승인 필요·재량) ②`CMBatchedSensorManager`로 원본 IMU 고주파 배치(800Hz accel·200Hz motion, 전송계약과 합치). 둘 다 **watchOS 앱** 전제 → **진짜 병목 = watchOS 앱 유무**(iPhone-only면 둘 다 막힘). 미확인=승인·24/7 지속성·앱 유무(김민지 확인 대기). 어느 쪽이든 우리 L2가 주력/안전망, 네이티브는 보강. (헴록 교훈: 구두/파생 출처 '확정'을 1차 소스가 정정.)"));
children.push(Bul("**★전송 계약**(워치 1초·폰 3초 배치): 센서는 ≥50Hz로 측정하되 배치로 전송. ★**낙상은 충격 전후 ~2초 고주파 윈도우가 한 배치에 담겨야** 동작한다 — '1샘플/3초'식 희소 전송이면 충격 파형이 소실돼 L2가 깨진다. 전송 아키텍처(워치→서버 직결 vs 폰 경유)·권한은 인형 님과 미결. 이를 README §2(전송 계약)에 명시."));

children.push(H2("6-20. 기기 구성 벤치마크 — 워치만 / 폰만 / 둘다 (v0.22)"));
children.push(P("질문: 같은 낙상을 워치만·폰만·둘다 가져왔을 때 탐지가 어떻게 달라지나? UMAFall은 한 trial(같은 낙상)을 손목·허리에서 동시 기록 → 정확한 짝비교 가능. 배포 모델이 UMAFall을 학습에 쓰므로 인샘플 방지 위해 **피험자 분리 5-fold**(테스트 피험자 미학습 후 추론). scripts/benchmark_device_configs.py.", { italics: true }));
children.push(P("UMAFall 617 trial(낙상 189/ADL 428·피험자 18) · 임계 워치 0.30/폰 0.40 · 융합=OR(L5 동일):", { bold: true }));
children.push(table(["구성", "recall", "오탐율", "precision", "F1"], [
  ["① 워치만(손목)", "0.984", "0.091", "0.827", "0.899"],
  ["② 폰만(허리)", "0.984", "0.005", "0.989", "0.987"],
  ["③ 둘다(OR)", "0.995", "0.096", "0.821", "0.900"],
], [2960, 1600, 1600, 1600, 1600]));
children.push(Bul("**둘다(OR)=recall 최고(0.995, 189중 1개만 놓침)** — 워치 놓친 낙상 2개를 폰이, 폰 놓친 2개를 워치가 건짐(상호보완 실측). 단 OR은 워치 오탐을 물려받아 오탐율 9.1→9.6%."));
children.push(Bul("**★양쪽 동시발화(교차확인)=ADL 오탐 0/428** + 낙상 184/189 회수. 이게 L5가 '둘 다 발화 → 고신뢰'로 쓰는 이유 — 둘이 동의하면 사실상 확정. '둘다'의 진짜 가치는 OR-recall + 0-오탐 고신뢰 채널 둘 다."));
children.push(Bul("**폰(허리)이 본질적으로 훨씬 정밀**(오탐 0.5% vs 9.1%·F1 0.987 vs 0.899) — 손목은 낙상유사 동작(팔 휘두름) 多. LODO(손목 0.74/허리 0.90)와 일관. **그럼에도 워치가 주력인 이유는 정확도가 아니라 가용성**(워치=상시 착용, 폰=간헐·주머니)."));
children.push(Bul("**목적 부합 방향 = 계층형 융합**(기기 강제 선택 아님): 양쪽 발화→즉시 고신뢰 격상 / 워치 단독→self-check 확인(9% 오탐을 확인루프로 흡수) / 폰 단독→경량 확인(정밀 0.99). 폰 부재(흔함)→워치 단독이 바닥(recall 0.984)+net 유지. = 기존 L5 설계의 정량 검증."));
children.push(Bul("**회색지대 교차확인 검증 → 기각(설계 유지)**: '워치·폰이 둘 다 약한 의심(0.15≤proba<임계)이면 격상하자'는 직관을 측정 → 회색지대는 거의 전부 ADL(워치 precision 0.05·폰 0.06), ★양쪽 회색 동시 = 단 2 trial·둘 다 ADL·낙상 0. 두 기기 약한 합의는 낙상 신호가 아니라 '양쪽 다 헷갈린 일상 동작'. → 자동 격상 규칙 추가는 과적합(넣었다면 오경보 2·회수 0). **회색지대=self_check 현 설계가 옳음**(D36 패턴: 분석이 기존 설계 검증). 참고로 양쪽 *확신* 동시는 precision 1.000(184 trial·ADL 0)=진짜 강신호, 이미 immobility 게이트로 처리."));
children.push(Bul("**한계(정직)**: 피험자 분리는 '미지의 피험자'지 '미지의 데이터셋'이 아님 → cross-dataset보다 낙관(보수적 하한=손목 LODO 0.74). UMAFall=14~68세 시뮬레이션 낙상(실 고령 낙상 아님). **고령 recall 갭(0.84)은 이 벤치마크로 해결 안 됨 — 데이터 문제(능동학습·POC).** 기기 융합은 가용성·신뢰도를 높일 뿐 고령 데이터 공백을 메우지 못함."));

children.push(H2("6-21. 디바이스 실태(앱 코드) + 전송 스펙·레이트 강건성 (v0.24)"));
children.push(P("hemlo 지적('우리 깃 코드를 보면 되잖아')으로 앱 repo GB-able/WIDYU-widyu(Flutter)를 직접 확인. 추정·구두답변이 아닌 1차 소스.", { italics: true }));
children.push(Bul("**★현 시점 최대 갭 — 디바이스 IMU 미구축**: 현재 앱은 어느 플랫폼에서도 가속도·자이로를 수집/전송하지 않음(심박만, Wear OS Health Services HEART_RATE_BPM 5초 배칭→Wear Data Layer→폰→서버). IMU·낙상·CMFall 코드 전무(전 repo grep 0). 현 워치=Wear OS(애플워치 아님), 애플워치 앱은 미완(WIP). → **L2 낙상은 라이브 데이터 소스가 없음** — iOS 네이티브 API 논쟁은 시기상조, 1순위는 IMU 파이프라인 구축."));
children.push(Bul("**IMU 전송 계획(김민지)**: 워치(심박+IMU) 1초/회·폰(위치+IMU 원본) 3초/회(위급시 1초). '낙상데이터'=플래그 아닌 원본 가속도·자이로(서버=WidU 판정) → WidU 설계와 합치."));
children.push(Bul("**★전송 스펙 핵심 — 간격 아닌 '배치 내 샘플레이트'**: WidU는 50Hz(FS)·2초창(WIN_SEC)·충격 1.8g(IMPACT_G)·이후 8초 무활동(POST_IMMOBILE_SEC) 요구. 1초/3초 배치 간격은 OK, 단 **배치 안에 ≥50Hz 샘플이 다 담겨야 함**(폰 3초=150샘플). '3초 1샘플'이면 충격(수십 ms) 소실→낙상 통째 깨짐. 권장=온디바이스 충격 트리거(|a|>1.8g)→±2초 50Hz 버스트+이후 8초(평상시 저레이트=배터리)."));
children.push(P("전송 레이트 강건성 측정(benchmark_transmission_rate.py — 모델은 50Hz 학습, 테스트 입력만 R Hz 열화):", { bold: true }));
children.push(table(["레이트", "워치만 R/F1", "폰만 R/F1", "둘다 R/F1"], [
...
children.push(Bul("**스펙(50Hz) 충족 시 성능 손실 0** — 50Hz 행이 기존 device 벤치마크와 숫자까지 동일(모델이 학습한 바로 그 레이트). **워치(손목)가 레이트에 가장 민감**(25Hz서 recall 0.91·12.5Hz서 0.72 붕괴) → 워치 반드시 ≥50Hz. 폰(허리)·둘다는 강건(폰이 받침). 한계: 샘플레이트 1축만(충격창 보존·타임스탬프 연속성은 별개 필수)."));
...
children.push(Bul("**★iOS 네이티브 낙상 — 조건부 가능(v0.23 정정)**: 'iOS 미가용'(v0.21)을 애플 1차 문서로 재검증 → iPhone 앱 단독은 불가하나 **watchOS 컴패니언 앱**이면 가능: CMFallDetectionManager(엔타이틀먼트 애플 승인 필요)·CMBatchedSensorManager(원본 IMU 800Hz 배치). 진짜 병목=watchOS 앱 유무. 미확인=승인·24/7 지속성·WIDYU watchOS 앱 유무(김민지 확인 대기). 승인 안 나도 우리 L2 동작 → 모델이 주력/안전망. 손목 recall 갭(0.74)은 여전히 L5 교차검증·미회복 net·능동학습으로 보완."));
children.push(Bul("**★전송 계약 의존(v0.21)**: 워치 1초·폰 3초 배치 전송. 낙상 검출은 충격 전후 고주파 윈도우가 배치에 온전히 담겨야 동작 — 희소/불규칙 전송이면 L2가 깨진다. 전송 아키텍처·권한은 인형 님과 미결(디바이스 노드 열린이슈)."));
...
children.push(table(["일자", "버전", "변경/결정", "비고"], [
  ...
  [today, "v0.21", "WIDYU 프론트 통화(6/30) 반영. 대부분 이미 일치, 정정 2건: ★iOS 네이티브 낙상 알림 미가용→우리 L2 모델 iOS 주력 ★전송 계약 워치1s/폰3s 배치(충격 고주파창 필수). 개인화 3일=L1 EWMA 정렬", "D39. 코드 로직 불변(pipeline 주석·README §2/§4/§12·brain 정정). 골든·reverify 영향 없음(동작 무변경). 전송 아키텍처는 인형 님과 미결."],
  [today, "v0.22", "기기 구성 벤치마크(워치만/폰만/둘다). UMAFall 동기 손목·허리·피험자분리. 워치만 F1 0.899(오탐9.1%)·폰만 0.987(0.5%)·둘다 OR recall 0.995. ★양쪽발화 ADL오탐 0/428. → 계층형 융합이 목적 부합(기존 L5 검증)", "D40. benchmark_device_configs.py(신규, 분석전용). 폰=본질정밀·워치=가용성 주력. 고령 recall 갭은 불변(데이터 문제). 서빙 코드 무변경(golden 11/11 유지). +회색지대 교차확인 격상 가설 측정→기각(양쪽 회색 동시 2건 전부 ADL)."],
  [today, "v0.23", "iOS 네이티브 낙상 '미가용'(D39)→'watchOS 앱 조건부 가능'으로 정정(애플 1차 문서 검증). iPhone 앱 단독 불가하나 watchOS 앱이면 CMFallDetectionManager(엔타이틀먼트)+CMBatchedSensorManager(원본 IMU 배치) 가능", "D41. 직전 정정은 구두 출처(파생)였음→1차 문서가 재정정. 진짜 병목=watchOS 앱 유무. 미확인=승인·지속성·앱유무(김민지 확인 대기). pipeline 주석·README §4/§12·brain 정정. 코드 로직 불변(golden 11/11)."],
  [today, "v0.24", "앱 코드(GB-able/WIDYU-widyu) 직접 확인 → ★디바이스 IMU 미구축(심박만, Wear OS)=현 최대 갭, 애플워치 미완. iOS API 논쟁 시기상조→1순위 IMU 파이프라인. 전송 스펙(배치 내 ≥50Hz)+레이트 강건성 측정: 50Hz=성능 유지, 워치는 <50Hz서 붕괴", "D42. hemlo '깃 코드 보면 되잖아' 지적이 레버. benchmark_transmission_rate.py 신규. README §12·pipeline 주석·brain 갱신. 워치 반드시 ≥50Hz·폰/둘다 강건. 서빙 코드 무변경(golden 11/11)."],
], [1300, 900, 5160, 2000]));
children.push(P(""));
