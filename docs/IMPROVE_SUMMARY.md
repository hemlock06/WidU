...
| **P1-2** | 입력검증 → 400 | 누락 필드(`KeyError`)·잘못된 enum/숫자(`ValueError`)·null/타입(`TypeError`)을 전역 errorhandler 로 **400 + 에러 본문** 변환. `serving/api.py` | 신규 4 테스트(bpm 누락·잘못된 accuracy·잘못된 kind·비숫자 bpm → 400) |
...
- `pytest tests/` → **39 passed, 0 xfailed** (베이스라인 27 passed·1 xfailed → healthz xfail 해소 + 신규 11 테스트)
...
2. **새 컨텍스트 적대적 diff 리뷰 1패스**(서브에이전트, 버그·회귀·요구오해만) 수행.
   ...
   - 지적 2건(우려, 버그 아님) 처리:
     ...
     - (A) 전역 errorhandler 가 내부 500급 오류를 400으로 가릴 수 있음(관측성 저하) → **유지**(스펙 P1-2 가 명시 요구·정상 입력엔 무영향). 더 외과적인 대안(경계 검증 헬퍼)은 `IMPROVE_PROPOSALS.md` 에 기록.
...
- **전역 errorhandler(A)**: 관측성 트레이드오프 존재(위 3-(A)).

## 5. 미구현 — 제안만 (`docs/IMPROVE_PROPOSALS.md`)

P0-1(상태 영속성), P0-2(인증·인가), P1-6(푸시 전달), P2-5(SLEEP/STEPS 무반응 처리 결정), + P1-2 경계검증 대안, + 3.9 호환 핀 전략. 설계결정·독립검증 곤란으로 코드 미잔류.
